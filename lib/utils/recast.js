...
// TODO: memberExpression みたいなのを MemberExpressionに直す。
  ...
  return builderName.replace(/^[a-z]+/, function (lowerCasePrefix) {
    ...
    switch (len) {
      ...
      default:
        ...
        // all but the last one, so that XMLDefaultDeclaration (for
        // example) becomes xmlDefaultDeclaration.
        return lowerCasePrefix.slice(
          0, len - 1).toUpperCase() +
          lowerCasePrefix.charAt(len - 1)
    }
  })
}

// get buildParams from def
export const getBuildParams = (builderName) => {
  const nodeType = getTypeName(builderName)
  return def(nodeType).field('type', String).buildParams
}
