...
export default (option = {}) => {
  const transform = (node) => {
    ...
    if (NODE_TYPE === 'CallExpression') {
      const args = _.map(node.arguments, (argument) => `<Value>${argument.source()}</Value>`).join('\n')
      // if nested FnCall
      if (n.MemberExpression.check(node.callee) && n.CallExpression.check(node.callee.object)) {
        return node.update(`
          <FnCall callee="${node.callee.property.name}">
            ${node.callee.source()}
            ${args}
          </FnCall>
        `)
      }
      return node.update(`<FnCall callee="${node.callee.source()}">${args}</FnCall>`)
    } else if (NODE_TYPE === 'MemberExpression') {
      // if nested FnCall
      if (n.CallExpression.check(node.object)) {
        return node.update(`${node.object.source()}`)
      }
      return node.update(`${node.object.source()}.${node.property.source()}`)
    ...
    `)
      ...
      const elements = _.trim(_.map(node.elements, (element) => element.source()).join(','))
      let withExpressionStatement = false
      if (n.ExpressionStatement.check(node.parent)) {
        withExpressionStatement = true
      }
      return node.update(`<Value ${withExpressionStatement && 'es'}>{[${_.isEmpty(node.elements) ? '' : elements}]}</Value>`)
