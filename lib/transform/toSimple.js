...
export default (option = {}) => {
  const transform = (node) => {
    ...
    if (NODE_TYPE === 'CallExpression') {
      ...
      return node.update(`<FnCall callee="${node.callee.source()}" ${withES ? 'es' : ''}>${args}</FnCall>`)
    ...
    `)
      ...
      const properties = _.reduce(node.properties, (result, property) => {
        transform(property.value) // call update first.
        result[property.key.name] = property.value.source()
        return result
      }, {})
      const withES = n.ExpressionStatement.check(node.parent)
      return node.update(`
    ...
    if (NODE_TYPE === 'VariableDeclarator') {
      ...
      if (node.declarations.length === 1 && n.VariableDeclarator.check(_.first(node.declarations))) {
        ...
        if (n.Literal.check(declarator.init)) {
          ...
          transform(declarator.init) // transform first to get source()
          declarations = [`${declarator.init.source()}`]
        }
      }
