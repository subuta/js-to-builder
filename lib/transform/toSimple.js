...
export default (option = {}) => {
  ...
  const transform = (node) => {
    ...
    if (NODE_TYPE === 'CallExpression') {
      ...
      return node.update(`${node.object.source()}.${node.property.name}`)
    } else if (NODE_TYPE === 'ArrowFunctionExpression') {
      const params = _.map(node.params, (param) => param.source()).join('\n')
      return node.update(`<ArrowFn ${node.async ? 'async' : ''} ${node.generator ? 'generator' : ''}>${params}${node.body.source()}</ArrowFn>`)
    ...
    if (NODE_TYPE === 'ClassDeclaration') {
      return node.update(`<ClassDef id="${node.id ? node.id.name : 'null'}">${node.superClass ? node.superClass.source() : ''}${node.body.source()}</ClassDef>`)
      ...
      if (n.Expression.check(node.key)) {
        return node.update(`<Method kind="${node.kind}" ${node.static ? 'static' : ''}>${node.key.source()}${node.value.source()}</Method>`)
      }
      return node.update(`<Method kind="${node.kind}" key={${node.key.source()}} ${node.static ? 'static' : ''}>${node.value.source()}</Method>`)
    }
