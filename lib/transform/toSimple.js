...
export default (option = {}) => {
  ...
  return (path) => {
    ...
    const replace = (jsx) => path.replace({...path.node, jsx})
    ...
    if (NODE_TYPE === 'callExpression') {
      ...
      if (n.MemberExpression.check(calleeNode) && n.CallExpression.check(calleeNode.object)) {
        return replace(`
          <FnCall callee="${node.callee.property.name}" ${commentPropsString} ${withES ? 'es' : ''}>
            ${node.callee.jsx}
            ${args}
          </FnCall>
        `)
      ...
      if (!callee) {
        return replace(`<FnCall ${commentPropsString} ${withES ? 'es' : ''}>${node.callee.jsx}${args}</FnCall>`)
      }

      return replace(`<FnCall ${callee ? `callee="${callee}"` : ''} ${commentPropsString} ${withES ? 'es' : ''}>${args}</FnCall>`)
      ...
      return replace(`<Value identifier="${stringify(node)}" ${commentPropsString} ${withES ? 'es' : ''}/>`)
      ...
      let properties = _.transform(node.properties, (result, property) => {
        ...
        if (n.Identifier.check(property.value)) {
          value = `<Value identifier="${property.value.name}" />`
        } else if (n.Literal.check(property.value)) {
          value = property.value.raw
      ...
      return replace(`
        <Value ${commentPropsString} ${withES ? 'es' : ''}>{${properties}}</Value>
      `)
    } else if (NODE_TYPE === 'arrayExpression') {
      const elements = _.trim(_.map(node.elements, (element) => element.raw).join(','))
      const withES = n.ExpressionStatement.check(path.parentPath.node)
      const commentPropsString = extractCommentPropsString(path.parentPath.node)
      return replace(`<Value ${commentPropsString} raw ${withES ? 'es' : ''}>{[${_.isEmpty(node.elements) ? '' : elements}]}</Value>`)
    ...
    `)
      ...
      const commentPropsString = extractCommentPropsString(path.parentPath.node)
      return replace(`
      <assignmentExpression operator="${node.operator}" ${commentPropsString} ${withES ? 'es' : ''}>
        ${node.left.jsx}
        ${node.right.jsx}
    ...
    `)
      ...
      const commentPropsString = extractCommentPropsString(path.parentPath.node)
      return replace(`
      <logicalExpression operator="${node.operator}" ${commentPropsString} ${withES ? 'es' : ''}>
        ${node.left.jsx}
        ${node.right.jsx}
      </logicalExpression>
      `)
    ...
    if (NODE_TYPE === 'literal') {
      ...
      const withES = n.ExpressionStatement.check(path.parentPath.node)
      const commentPropsString = extractCommentPropsString(path.parentPath.node)
      return replace(`<identifier ${withES ? commentPropsString : ''} ${withES ? 'es' : ''} >${node.name}</identifier>`)
    }
    ...
    if (NODE_TYPE === 'importDeclaration') {
      ...
      if (node.specifiers.length > 1 || n.ImportSpecifier.check(specifier)) {
        ...
        return replace(`<Import name="${specifier.local.name}" source="${node.source.value}" ${isDefault ? 'default' : ''}/>`)
      }
