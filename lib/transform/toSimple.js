...
export default (option = {}) => {
  ...
  return (path) => {
    ...
    if (NODE_TYPE === 'callExpression') {
      ...
      return replace(`<Value identifier="${stringify(node)}" ${withES ? 'es' : ''}/>`)
      ...
      const withES = n.ExpressionStatement.check(path.parentPath.node)
      return replace(`<Value raw ${withES ? 'es' : ''}>{[${_.isEmpty(node.elements) ? '' : elements}]}</Value>`)
    ...
    `)
      ...
      const withES = n.ExpressionStatement.check(path.parentPath.node)
      return replace(`
      <logicalExpression operator="${node.operator}" ${withES ? 'es' : ''}>
        ${node.left.jsx}
        ${node.right.jsx}
      </logicalExpression>
      `)
