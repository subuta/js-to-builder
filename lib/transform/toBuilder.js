...
export default (option = {}) => (node) => {
  ...
  if (NODE_TYPE === 'CallExpression') {
    ...
    return node.update(`
      <callExpression>${node.callee.source()}${args}</callExpression>
    `)
    ...
    return node.update(`
      <arrowFunctionExpression ${node.async ? 'async' : ''} ${node.generator ? 'generator' : ''}>${params}${node.body.source()}</arrowFunctionExpression>
    `)
    ...
    const params = _.map(node.params, (param) => param.source()).join('\n')
    return node.update(`
      <functionExpression ${node.id ? `id={${node.id.source()}}` : ''} ${node.async ? 'async' : ''} ${node.generator ? 'generator' : ''}>
        ${params}${node.body.source()}
      </functionExpression>
    `)
  } else if (NODE_TYPE === 'ThisExpression') {
    return node.update(`<thisExpression />`)
  }
  ...
  if (NODE_TYPE === 'Property') {
    ...
    // FIXME: assignmentPatternとPropertyのstart,endが同じ問題
    // 下記のworkaroundを直す。
  ...
  if (NODE_TYPE === 'YieldExpression') {
    return node.update(`<yieldExpression>${node.argument.source()}</yieldExpression>`)
  }

  if (NODE_TYPE === 'ClassDeclaration') {
    return node.update(`<classDeclaration id="${node.id ? node.id.name : 'null'}">${node.superClass ? node.superClass.source() : ''}${node.body.source()}</classDeclaration>`)
  } else if (NODE_TYPE === 'ClassExpression') {
    const args = _.map(node.arguments, (argument) => argument.source()).join('\n')
    return node.update(`<classExpression>${args}</classExpression>`)
  } else if (NODE_TYPE === 'ClassBody') {
    const body = _.map(node.body, (row) => row.source()).join('\n')
    return node.update(`<classBody>${body}</classBody>`)
  } else if (NODE_TYPE === 'MethodDefinition') {
    if (n.Expression.check(node.key)) {
      return node.update(`<methodDefinition kind="${node.kind}" ${node.static ? 'static' : ''}>${node.key.source()}${node.value.source()}</methodDefinition>`)
    }
    return node.update(`<methodDefinition kind="${node.kind}" key="${node.key.source()}" ${node.static ? 'static' : ''}>${node.value.source()}</methodDefinition>`)
  }

  // ES7
  if (NODE_TYPE === 'AwaitExpression') {
    return node.update(`<awaitExpression>${node.argument.source()}</awaitExpression>`)
  }
