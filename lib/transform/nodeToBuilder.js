...
export default (option = {}) => (node) => {
  ...
  if (NODE_TYPE === 'CallExpression') {
    ...
    return node.update(`b.binaryExpression('${node.operator}', ${node.left.source()}, ${node.right.source()})`)
  }
  ...
  if (NODE_TYPE === 'ExpressionStatement') {
    ...
    return node.update(`b.ifStatement(${node.test.source()}, ${node.consequent.source()}, ${node.alternate ? node.alternate.source() : null})`)
  }
