...
export const Value = withES((props) => {
  ...
  const {children, identifier, raw} = props
      ...
      <objectExpression>
        {_.map(value, (v, k) => {
          ...
          if (!_.isPlainObject(v)) {
            v = <literal>{v}</literal>
          } else if (n.Identifier.check(v)) {
            // use shorthand if equals.
            shorthand = _.isEqual(k, v)
          }

          return (
            <property kind='init' method={false} shorthand={shorthand} computed={false}>
              {k}{v}
            </property>
          )
        })}
  ...
  if (identifier) {
    ...
    return (function parseMember (identifiers) {
      let property = fromStr(identifiers.pop())

      // if not has extra params.
      ...
      if (identifiers.length > 1) {
        object = parseMember(identifiers)
      } else {
        object = fromStr(identifiers.shift())
      }

      let computed = n.Literal.check(object)
      return (
  ...
  if (_.isArray(children) && (children.length > 1 || raw)) {
    return (
      <arrayExpression>
...
export const Declarator = (props) => {
  ...
  if (props.value) {
    value = (
      <Value raw={raw}>{props.value}</Value>
    )
  } else {
