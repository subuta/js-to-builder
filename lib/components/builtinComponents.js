...
// NOTE: ast-typesのbuildの順で引数を渡す

// AST(JSX) to builder
export const program = ({children}) => {
  return b.program.apply(this, [_.flattenDeep(children)])
}
...
export const thisExpression = () => {
  return b.thisExpression.apply(this, [])
}
...
export const literal = ({children}) => {
  return b.literal.apply(this, _.isEmpty(children) ? [''] : children)
}
...
export const classDeclaration = (props) => {
  const {children} = props
  const id = props.id ? b.identifier(props.id) : null
  let superClass = null
  if (children.length === 2) {
    superClass = children.shift()
  }
  return b.classDeclaration.apply(this, [id, ...children, superClass])
}

export const classExpression = ({children}) => {
  return b.classExpression.apply(this, children)
}

export const classBody = ({children}) => {
  return b.classBody.apply(this, [children])
}

export const methodDefinition = (props) => {
  const {kind, children} = props
  const isStatic = props.static || false
  const key = props.key ? props.key : children.shift()
  return b.methodDefinition.apply(this, [kind, key, ...children, isStatic])
}
